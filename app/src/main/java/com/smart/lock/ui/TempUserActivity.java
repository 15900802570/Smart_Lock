...
public class TempUserActivity extends AppCompatActivity implements View.OnClickListener, ViewPager.OnPageChangeListener {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BleMsg.STR_RSP_MSG1E_ERRCODE)) {
                ...
                if (errCode[3] == 0x13) {
                    ...
                    ArrayList<DeviceUser> users = DeviceUserDao.getInstance(TempUserActivity.this).queryDeviceUsers(mDefaultDevice.getDeviceNodeId());
                    for (DeviceUser user : users) {
                        if (!(user.getUserId() == mDefaultDevice.getUserId())) {
                            DeviceUserDao.getInstance(TempUserActivity.this).delete(user);
                        }
                    }
