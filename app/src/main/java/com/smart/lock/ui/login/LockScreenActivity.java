        ...
        et_pwd4.setOnTextChangedListener(new PasswordTextView.OnTextChangedListener() {
            ...
            public void textChanged(String content) {
                ...
                if (type == ConstantUtil.SETTING_PASSWORD) {//设置密码
                    ...
                    if (!input.equals(SharedPreferenceUtil.getInstance().readString("password"))) {
                        showToastMsg("密码错误,请重新输入");
                    } else {
                        showToastMsg("密码正确,欢迎登录");
                        startActivity(new Intent(LockScreenActivity.this, LuckyActivity.class));
                        finish();
                    }
                    ...
                    if (input.equals(fBuffer.toString())) {//一致
                        ...
                        //保存密码到文件中
                        SharedPreferenceUtil.getInstance().initSharedPreferences(LockScreenActivity.this);
                        SharedPreferenceUtil.getInstance().writeString("password", input);
