                    ...
                    if (mDevice.getState() == Device.BLE_CONNECTED) {
                        ...
                        mLoadDialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
                            ...
                            public boolean onKey(DialogInterface dialogInterface, int i, KeyEvent keyEvent) {
                                if (keyEvent.getAction() == KeyEvent.ACTION_DOWN && i == KeyEvent.KEYCODE_BACK) {
                                    LogUtil.d(TAG, "按了返回键");
                                    mCancelDialog = DialogUtils.createTipsDialogWithConfirmAndCancel(mCtx, getString(R.string.cancel_warning) + getString(R.string.fingerprint) + getString(R.string.modify));
