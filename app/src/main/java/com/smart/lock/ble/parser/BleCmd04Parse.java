...
public class BleCmd04Parse implements BleCommandParse {
    ...
    public Message parse(byte[] cmd) {
        ...
        byte[] tmpPwdSk;
        byte[] userState;
        byte[] enableStatus = new byte[1]; //   启用的设备
        byte[] stStatus2 = new byte[1]; // 设备状态
        ...
        if (mIs128Code) {
            ...
            System.arraycopy(buf, 84 + ConstantUtil.ADMIN_USR_NUM + ConstantUtil.TMP_USR_NUM - 1, enableStatus,0, 1); //最后一个字节
            System.arraycopy(buf, 84 + ConstantUtil.ADMIN_USR_NUM + ConstantUtil.TMP_USR_NUM - 2, stStatus2,0, 1); //倒数第二个字节
            ...
            System.arraycopy(buf, 148 + ConstantUtil.ADMIN_USR_NUM + ConstantUtil.TMP_USR_NUM - 1, enableStatus,0, 1); // 原临时用户最后一个字节
            System.arraycopy(buf, 148 + ConstantUtil.ADMIN_USR_NUM + ConstantUtil.TMP_USR_NUM - 2, stStatus2,0, 1); // 原临时用户倒数第二个字节
        ...
        return MessageCreator.getCmd04Message(getParseKey(), batPerscent[0], syncUsers, userStatus[0], stStatus[0], unLockTime[0], tmpPwdSk, userState, powerSave, enableStatus[0],stStatus2[0]);
    }
