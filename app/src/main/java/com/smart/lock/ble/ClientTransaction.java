...
public class ClientTransaction implements TimerListener, BleMessageListener {
	 ...
	 * @param pnasProvider
	 */
	public ClientTransaction(Message message, int timeout, BleMessageListener agent, BleProvider pnasProvider) {
		mMessage = message;
		 Log.d(TAG,"message is " + message.toString());
		mTimeout = timeout;
		listener = agent;
		BleMsgProvider = pnasProvider;
	...
	@Override
	public void onReceive(BleProvider pnasProvide, Message message) {
		listener.onReceive(pnasProvide, message);
	}
